...
import { ClothingType, ItemTable } from "@/type";
...
const handleInputToSuggestions = async (
  clothing_type: ClothingType,
  image_url: ImageURL | null,
  textual_info: string | null,
  ...
  try {
    ...
    if (image_url && textual_info) {
      const response = await chatCompletionTextAndImage(
        model,
        textual_info,
        image_url
      );
      if (response) {
        suggestedLabelStrings = response
          .split("\n")
      ...
      const response = await chatCompletionTextOnly(
        model,
        labelsArray.join(" ")
      );
      if (response) {
...
const handleRequest = async (
  ...
  try {
    const suggestedLabelStrings = await handleInputToSuggestions(
      image_url,
      textual_info,
      max_num_suggestion
    );
    ...
    const matchedItemIds = await handleSuggestionMatching(
