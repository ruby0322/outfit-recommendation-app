...
export default function SearchPage() {
  ...
  useEffect(() => {
    (async () => {
      if (searchParams.get('label_string') && searchParams.get('gender') && searchParams.get('title')) {
        ...
        setUserId(userResponse?.id as string);
        const res = await handleSearch(searchParams.get('label_string') as string, searchParams.get('gender'), page, priceLowerBound?priceLowerBound:undefined, priceUpperBound?priceUpperBound:undefined, provider?provider:undefined, undefined, userResponse?.id);
  ...
  const onSubmit = async () => {
    ...
    const res = await handleSearch(label, gender, page, priceLowerBound?priceLowerBound:undefined, priceUpperBound?priceUpperBound:undefined, provider?provider:undefined, undefined, userId as string);
    setResults([...(res?.series as Series[])] as Series[]);
    setTotalPages(res?.totalPages as number);
    setPage(1);
  ...
  const handlePageNavigation = async (page: number) => {
    ...
    const res = await handleSearch(labelString, gender, page, priceLowerBound?priceLowerBound:undefined, priceUpperBound?priceUpperBound:undefined, provider?provider:undefined, undefined, userId as string);
    console.log(res);
    setResults([...(res?.series as Series[])] as Series[]);
    setLoading(false);
  };
